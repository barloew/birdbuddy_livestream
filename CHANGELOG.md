...
- The holding pattern appears as soon as you open the stream, instead of after
  a wait
- A running stream is no longer interrupted while it is healthy
- Only feeder states that genuinely rule out a stream block one; the status
  says which state that is
- The stream URL is refreshed when it is about to be used, so opening the camera
  after a quiet period gives a picture straight away
- go2rtc addresses without a scheme or port are accepted
